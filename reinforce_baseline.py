...
class Agent():
    ...
    def __init__(self, learning_rate, discount, state_size, action_size, hidden_size = 128):
        ...
        self.value_network = Value(state_size, hidden_size).to(device)
        self.optimizer_vn = optim.Adam(self.value_network.parameters(), lr = learning_rate)
    ...
    def train(self, state_list, action_list, reward_list):
        ...
        loss_fn_vn = nn.MSELoss()
        vn_loss = loss_fn_vn(value_t, return_t)
        self.optimizer_vn.zero_grad()
        vn_loss.backward()
        self.optimizer_vn.step()

        return loss.detach().cpu().numpy(), vn_loss.detach().cpu().numpy()
